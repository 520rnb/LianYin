...
$(function () {
	...
	var all = permissions(sidebar.data, 30101)
	// 开始的时间和结束的时间
	var beginDate = '';
	var endDate = '';
	var obj = {
	...
	$(document).on('click', '.checked-all', function () {
		...
		if (falg) {
			$('tbody').find('.check-item').prop('checked', true)
		} else {
			$('tbody').find('.check-item').prop('checked', false)
		}
	})
	// tbody 下面的tr注册点击事件
	...
	$(document).on('click', '.recommend-btn', function () {
		var dynamicId = $(this).attr('ids')
		$('#recommended-tips').modal('show')
		$('.btn-recommended').attr('dynamicId', dynamicId)
	})
	...
	// 动态列表新增动态
	$(document).on('click', '.add-dynamic', function(){
